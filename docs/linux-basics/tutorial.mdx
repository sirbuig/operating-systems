...
![Linux File System](/img/week1-linux/LinuxFileSystem.png)
...
If `folder2` were empty, the command would have worked fine. Since it's not, so we must pass `-r` (for `--recursive`) for it to work.
...
![](/img/week1-linux/tux_lolcat.png)

## Further Reading

- [Absolute and relative paths](https://www.redhat.com/en/blog/linux-path-absolute-relative)
- [Root Folder explained](https://averagelinuxuser.com/linux-root-folders-explained/)
- [`cat` command and operators in action](https://hostman.com/tutorials/linux-cat-command/)
